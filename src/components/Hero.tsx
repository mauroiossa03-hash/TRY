...
export default function Hero() {
  ...
  useEffect(() => {
    ...
    const onChange = () => setReduceMotion(reduceMotionMq.matches);
    reduceMotionMq.addEventListener("change", onChange);
    return () => {
      reduceMotionMq.removeEventListener("change", onChange);
    };
  }, []);
  ...
  // invoked from JS with the element confirmed muted. Force it on mount.
  useEffect(() => {
    if (!showVideo) return;
